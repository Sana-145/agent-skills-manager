...
export default class M extends Migration<Start, End> {
  ...
  override get operations() {
    return [
      this.dataTransform(contract, 'typechange-skills-description', {
        check: () => db.public.skills.select('id').where((f, fns) => fns.eq(f.description, null)).limit(1),
        run: () => db.public.skills.update({ description: '' }).where((f, fns) => fns.eq(f.description, null)),
      }),
      ...
      this.dataTransform(contract, 'typechange-skills-name', {
        check: () => db.public.skills.select('id').where((f, fns) => fns.eq(f.name, null)).limit(1),
        run: () => db.public.skills.update({ name: '' }).where((f, fns) => fns.eq(f.name, null)),
      }),
      this.alterColumnType({
        schema: 'public',
        table: 'skills',
